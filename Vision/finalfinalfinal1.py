...
def threshold_frame(frame):
    ...
    black_lower = np.array([0, 0, 0])
    black_upper = np.array([179, 255, 110])

    # white_lower = np.array([0, 0, 197])
    # white_upper = np.array([179, 85, 255])
    ...
    white_upper = np.array([33, 91, 255])
...
def img_process(frame,grid_coordinates):
    ...
    for contour in white_contours:
        if cv2.contourArea(contour) > 300 and cv2.contourArea(contour) < 600:  # 过滤掉小噪声
            perimeter = cv2.arcLength(contour, True)
            area = cv2.contourArea(contour)
            circularity = 4 * np.pi * (area / (perimeter * perimeter))
            print('111')
...
def detect_white_piece(white_centers, grid_coordinates, board):
    ...
    for (x, y) in white_centers:
        ...
        if closest_row != -1 and closest_col != -1:
            if board[closest_row, closest_col] != 2 and (closest_row, closest_col) not in white_piece_last_positions and board[closest_row, closest_col] !=1:
                ...
                last_white_update_time += 8
    return detected

# 引入一个计数器和时间戳来记录棋子的位置变化
black_update_interval = 12  # 设置时间间隔为1秒
...
def detect_black_piece(black_centers, grid_coordinates, board):
    ...
    for (x, y) in black_centers:
        ...
        if closest_row != -1 and closest_col != -1:
            # 确保位置没有被多次更新
            if board[closest_row, closest_col] != 1 and (closest_row, closest_col) not in black_piece_last_positions and board[closest_row, closest_col] !=2:
...
fourth_rev_flag = 0
rev_data_xiaweji = 0
def fourth_task(board, bisheng_flag_1, judge_mid_or_bian_1,sorted_white_centers_current,fourth_task_plack):
    global bisheng_flag
    ...
    if np.all(board == 0) and fourth_task_plack is not None:
        ...
        send_data(ser,10-fourth_task_plack)
                ...
                if rev_data_xiaweji_int == 16:
                    ...
                    if board[1, 1] == 1 and \
                        ...
                        if move:
                            ...
                            position_index = 10 - (3 * move[0] + move[1] + 1)
...
def fifth_task(board,bisheng_flag_1,judge_mid_or_bian_1,board_qizi):
    ...
    if judge_mid_or_bian_1 == 0:
        ...
        if fifth_rev_flag == 1:
            ...
            if rev_data_xiaweji_fifth is not None:
                ...
                if rev_data_xiaweji_fifth_int == 16:
                    print('rev_data_xiaweji_fifth 是 16')
                    fifth_rev_flag = 0
                    rev_data_xiaweji_fifth = 0
                    if board[1, 1] == 1 and \
                            (board[0, 0] == 0 and board[0, 1] == 0 and board[0, 2] == 0 and
                             board[1, 0] == 0 and board[1, 2] == 0 and
                             board[2, 0] == 0 and board[2, 1] == 0 and board[2, 2] == 0):
                        judge_mid_or_bian_1 = 2  # 中间的情况
                        jiance_again = 2
                        judge_mid_or_bian=judge_mid_or_bian_1  # 中间的情况
                    ...
                    for pos in [(0, 0), (0, 2), (2, 0), (2, 2)]:
                        ...
                        if board[pos[0], pos[1]] == 0:
                            ...
                            fifth_rev_flag = 0
                            rev_data_xiaweji_fifth = 0
                            position_index = 10-(3 * pos[0] + pos[1] + 1)
                            # 将 x 和 y 转换为字符串，并添加 \r\n 作为分隔符
                            message = f"A{position_index}B"  # 发送给下位机下第几个棋子
                            ser.write(message.encode('utf-8'))
                            print(f"下位机接收下棋: {position_index}, {1}")
                            print(f"放置边角: {position_index}, {1}")
                            back_normal = 1
                            break
                        ...
                        move = find_winning_move(board, 2)  # 1表示黑棋
                        if move:
                            board[move[0], move[1]] = 2  # 更新棋盘状态
                            print('win!!!!')
                            fifth_rev_flag = 0
                        ...
                        if white_pieces < black_pieces:
                            ...
                            if empty_positions:
                                ...
                                position_index = 10 - (3 * move[0] + move[1] + 1)
                                send_data(ser, position_index)
                                return (row, col),win_flag_1



    if (judge_mid_or_bian_1 == 1):
        if (detect_black_piece(sorted_black_centers_current, grid_coordinates, board)or jiance_again) and fifth_rev_flag == 0:
            fifth_rev_flag = 1
        else:
    ...
    if judge_mid_or_bian_1 == 2:
        ...
        if (detect_black_piece(sorted_black_centers_current, grid_coordinates, board) or back_normal) and fifth_rev_flag == 0:
            ...
            print("No new black pieces detected. Waiting...")
        print("第二部",judge_mid_or_bian_1)
        if fifth_rev_flag == 1:
            rev_data_xiaweji_fifth = rev_data(ser)
            print('rev_data_xiaweji_fifthrev_data_xiaweji_fifth受到的消息', rev_data_xiaweji_fifth)
            print('第二进行了')
            if rev_data_xiaweji_fifth is not None:
                rev_data_xiaweji_fifth_int = int(rev_data_xiaweji_fifth.decode('utf-8').strip())  # 解码并转换为整数
                if rev_data_xiaweji_fifth_int == 16:
...
while True:
    ...
    if yunxing_once <=10:
        ...
        #第四个任务
        if diyicishoudao == 0:
            diyicishoudao_biaozhiwei = rev_data(ser)
            print("任务接收中")
            if diyicishoudao_biaozhiwei is not None:
                diyicishoudao_biaozhiwei_int = int(diyicishoudao_biaozhiwei.decode('utf-8').strip())  # 解码并转换为整数
                print("任务收到")
                diyicishoudao = 1
        if diyicishoudao == 1:
            if diyicishoudao_biaozhiwei_int == 4:
                move,win_flag = fourth_task(board, bisheng_flag, judge_mid_or_bian,sorted_white_centers_current,1)
            #第五个任务
            elif diyicishoudao_biaozhiwei_int == 5:
                move,win_flag= fifth_task(board,bisheng_flag,judge_mid_or_bian,board_qizi)
        # print('Move to',move)
