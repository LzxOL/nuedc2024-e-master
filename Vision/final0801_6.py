...
cap.set(cv2.CAP_PROP_EXPOSURE, -5)  # 曝光 -4
...
def send_data(ser,position_index):
...
def threshold_frame(frame):
    ...
    white_lower = np.array([0, 0, 172])
    white_upper = np.array([179, 13, 255])
...
def img_process(frame,grid_coordinates):
    ...
    frame_qizi = cv2.erode(frame_qizi,(3,3),0)
    ...
    for contour in black_contours:
        if cv2.contourArea(contour) > 200 and cv2.contourArea(contour) < 600:  # 过滤掉小噪声
    ...
    for contour in white_contours:
        if cv2.contourArea(contour) > 300 and cv2.contourArea(contour) < 500:  # 过滤掉小噪声
            perimeter = cv2.arcLength(contour, True)
            area = cv2.contourArea(contour)
            circularity = 4 * np.pi * (area / (perimeter * perimeter))
            print('111')
            print(circularity)
            if 0.6 < circularity < 1.3:  # 判断是否为圆形
                cX, cY = get_center(contour)
...
def detect_black_piece(black_centers, grid_coordinates, board):
    global last_black_update_time, black_piece_last_positions,update_board_position
    detected = False
        ...
        for i in range(3):
            for j in range(3):
                ...
                if distance < min_distance:
                    ...
                    min_distance = distance
                    closest_row, closest_col = i, j
                    update_board_position = (closest_row, closest_col)
        if closest_row != -1 and closest_col != -1:
            # 确保位置没有被多次更新
...
fourth_rev_flag = 0
rev_data_xiaweji = 0
def fourth_task(board, bisheng_flag_1, judge_mid_or_bian_1,sorted_white_centers_current):
    global bisheng_flag
    global judge_mid_or_bian
    global ser
    global fourth_rev_flag,rev_data_xiaweji
    win_flag = 0
    back_normal = 0
    jiance_again = 0
    ...
    if np.all(board == 0) and fourth_task_plack is not None:
        ...
        send_data(ser,fourth_task_plack)
        return (row, col), win_flag

    print('fourth_task_plack', fourth_task_plack)
    print(board)
    ...
    if (judge_mid_or_bian_1 == 1):
        if detect_white_piece(sorted_white_centers_current, grid_coordinates, board) and fourth_rev_flag == 0:
            fourth_rev_flag = 1
            # position_index = 11
            # message = f"A{position_index}B"  # 发送给下位机下第几个棋子
            # ser.write(message.encode('utf-8'))
        ...
        if fourth_rev_flag == 1:
            rev_data_xiaweji = rev_data(ser)
            if rev_data_xiaweji is not None:
                rev_data_xiaweji_int = int(rev_data_xiaweji.decode('utf-8').strip())  # 解码并转换为整数
                if rev_data_xiaweji_int == 16:
                    print('rev_data_xiaweji 是 16')
                    ...
                    if board[1, 1] == 1 and \
                        ...
                        print('judge_mid_or_bian_1sadsada',judge_mid_or_bian_1)
                        # back_normal = 1
                        ...
                        if move:
                            ...
                            print('win!!!!')
                            win_flag = 1
                            position_index = 3 * move[0] + move[1] + 1
                            # 将 x 和 y 转换为字符串，并添加 \r\n 作为分隔符
                            message = f"A{position_index}B"  # 发送给下位机下第几个棋子
                            ser.write(message.encode('utf-8'))
                            print(f"下位机接收下棋: {position_index}, {1}")
                            rev_data_xiaweji = 0
                            fourth_rev_flag = 0
                        ...
                        if white_pieces <= black_pieces:
                            empty_positions = [(row, col) for row in range(3) for col in range(3) if
                                               board[row, col] == 0]
    ...
    if judge_mid_or_bian_1 == 2:
        if (detect_white_piece(sorted_white_centers_current, grid_coordinates, board) or back_normal) and fourth_rev_flag == 0:
            fourth_rev_flag = 1
            print('标志位为1')
        else:
        ...
        if fourth_rev_flag == 1:
            rev_data_xiaweji = rev_data(ser)
            print('rev_data_xiawejirev_data_xiawejirev_data_xiaweji',rev_data_xiaweji)
            print('111111111111111111111111111111116666666666')
            if rev_data_xiaweji is not None:
                rev_data_xiaweji_int = int(rev_data_xiaweji.decode('utf-8').strip())  # 解码并转换为整数
                if rev_data_xiaweji_int == 16:
                    print('rev_data_xiaweji 是 16')
    ...
    if judge_mid_or_bian_1 == 3:
        ...
        if (detect_white_piece(sorted_white_centers_current, grid_coordinates, board) or jiance_again )and fourth_rev_flag ==0:
            fourth_rev_flag = 1
            # jiance_again = 0
            #
    ...
    judge_mid_or_bian = judge_mid_or_bian_1
    print("finsda",judge_mid_or_bian_1)
    return None, win_flag
# ----------------------------------------------------------------------------------

def detect_moved_piece(black_centers_current, grid_coordinates):
    NewBoard = np.zeros((3, 3), dtype=int)  # 创建一个新的3x3的棋盘状态
    for (x, y) in black_centers_current:
        min_distance = 10
        closest_row, closest_col = -1, -1
        for i in range(3):
            for j in range(3):
                ...
                if distance < min_distance:
                    ...
                    closest_row, closest_col = i, j
        if closest_row != -1 and closest_col != -1:
            NewBoard[closest_row, closest_col] = 1  # 更新新的棋盘状态，1表示黑棋
    return NewBoard

#第五个任务 -----------------------------------------------------------------------------
fifth_rev_flag = 0
rev_data_xiaweji_fifth = 0
rev_change_position_msg = 0
change_position_flag = 0
update_board_position = None
def fifth_task(board,bisheng_flag_1,judge_mid_or_bian_1,board_qizi,white_centers_current,black_centers_current):
    global bisheng_flag
    global judge_mid_or_bian
    global count_cishu
    global fifth_rev_flag,rev_data_xiaweji_fifth
    global rev_change_position_msg,change_position_flag
    global update_board_position
    print('judge_mid_or_bian_1',judge_mid_or_bian_1)
    ...
    placed_pieces = [[None, None, None], [None, None, None], [None, None, None]]  # 存储已放置棋子的坐标
    previous_placed_pieces = [[None, None, None], [None, None, None], [None, None, None]]  # 存储之前已放置棋子的坐标

    # def update_placed_pieces():
    #     nonlocal previous_placed_pieces
    #     previous_placed_pieces = [row[:] for row in placed_pieces]  # 复制当前的placed_pieces
    #     for i in range(3):
    #         for j in range(3):
    #             if board[i, j] == 1 or board[i, j] == 2:
    #                 placed_pieces[i][j] = (grid_coordinates[0, i * 3 + j][0], grid_coordinates[0, i * 3 + j][1])
    #
    # def check_for_moved_piece():
    #     for i in range(3):
    #         for j in range(3):
    #             if placed_pieces[i][j] is not None and previous_placed_pieces[i][j] is not None:
    #                 if placed_pieces[i][j] != previous_placed_pieces[i][j]:
    #                     return previous_placed_pieces[i][j], placed_pieces[i][j], i, j  # 返回被移动的棋子的旧位置，新位置和位置索引
    #     return None

    if judge_mid_or_bian_1 == 0:
        # update_placed_pieces()
        # rev_change_position_msg = rev_data(ser)
        # if rev_change_position_msg is not None:
        #     rev_change_position_msg_int = int(rev_change_position_msg.decode('utf-8').strip())  # 解码并转换为整数
        #     if rev_change_position_msg_int == 14:
        #         change_position_flag = 1
        #         print('换位置了')

        if detect_black_piece(sorted_black_centers_current, grid_coordinates, board) and fifth_rev_flag== 0:
            fifth_rev_flag = 1
        else:
            print("等人下第一步棋子")

        if fifth_rev_flag == 1:
            rev_data_xiaweji_fifth = rev_data(ser)
            print('rev_data_xiaweji_fifthrev_data_xiaweji_fifth', rev_data_xiaweji_fifth)
            print('1111111111111111111111111115555555555555555555555555')

            rev_data_xiaweji_fifth_int = 14

            if rev_data_xiaweji_fifth is not None:
                rev_data_xiaweji_fifth_int = int(rev_data_xiaweji_fifth.decode('utf-8').strip())  # 解码并转换为整数
                if rev_data_xiaweji_fifth_int  == 15:
                    print('rev_data_xiaweji_fifth 是 15')
                    fifth_rev_flag = 0
                    rev_data_xiaweji_fifth = 0
                    if board[1, 1] == 1 and \
                            (board[0, 0] == 0 and board[0, 1] == 0 and board[0, 2] == 0 and
                ...
                #棋子动了动了 需要计算目前棋盘黑棋的所有坐标点和grid的关系，然后写一个New_board,再对比New_Board和board的区别，这种情况下应该是
                #New_Board比board少一个board[][] == 1的情况，这个board[][] == 1的格子就是移动的点,然后update_board_position作为新增的点，将然后update_board_position的点移动
                #回board[][] == 1的这个点
                elif rev_data_xiaweji_fifth_int == 14:
                    fifth_rev_flag = 0
                    rev_data_xiaweji_fifth = 0
                    new_board = detect_moved_piece(black_centers_current,grid_coordinates)
                    moved_piece = None
                    print('newboard')
                    for i in range(3):
                        for j in range(3):
                            if board[i][j] == 1 and new_board[i][j] == 0:
                                moved_piece = (
                                i, j, (grid_coordinates[0, i * 3 + j][0], grid_coordinates[0, i * 3 + j][1]))
                                break
                        if moved_piece:
                            break
                    if moved_piece:
                        old_row, old_col, old_position = moved_piece
                        update_board_position = old_position
                        board[old_row][old_col] = 1  # 恢复棋子位置
                        print(f"将 ({update_board_position[0]}, {update_board_position[1]}) 移动到 {old_row}, {old_col}")
    ...
    if (judge_mid_or_bian_1 == 1):
        if (detect_black_piece(sorted_black_centers_current, grid_coordinates, board)or jiance_again) and fifth_rev_flag:
            fifth_rev_flag = 1
        else:
    ...
    if judge_mid_or_bian_1 == 2:
        ...
        if (detect_black_piece(sorted_black_centers_current, grid_coordinates, board) or back_normal) and fifth_rev_flag == 0:
            fifth_rev_flag = 1
            # back_normal = 0
            # move = find_winning_move(board, 1)  # 1表示黑棋
            # if move:
...
# 粉色粉色
lower_yellow = np.array([90, 28, 132])
upper_yellow = np.array([179, 38, 255])
...
while True:
    ...
    if yunxing_once <=10:
        ...
        src_points = np.array([[201, 54], [201, 310], [470, 54], [470, 310]], dtype="float32")
        src_points = sort_points(src_points)
    else:
        if len(src_points) == 4:
            # 提取感兴趣区域 (ROI)
        ...
        #第四个任务
        move,win_flag = fourth_task(board, bisheng_flag, judge_mid_or_bian,sorted_white_centers_current)
        #第五个任务
        # move= fifth_task(board,bisheng_flag,judge_mid_or_bian,board_qizi,sorted_white_centers_current,sorted_black_centers_current)
        # print('Move to',move)
