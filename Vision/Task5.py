...
def fifth_task(board,bisheng_flag_1,judge_mid_or_bian_1):
    global bisheng_flag
    ...
    print('bisheng_flag:', bisheng_flag_1)
    print('bisheng_flag:', bisheng_flag_1)
    ...
    # if bisheng_flag_1 == 0 or bisheng_flag_1 == 4:
    #     # 如果没有需要防守的情况，随机选择一个空位
    #     for row in range(3):
    #         for col in range(3):
    #             if board[row, col] == 0:
...
while True:
    ...
    if yunxing_once <=10:
        ...
        if yunxing_once_1 <= 10:
            yunxing_once_1 += 1
            squares = find_squares(edges)
            # 按照你的代码逻辑，首先得到 final_sorted_centers
            centers = []
            for square in squares:
                cX, cY = get_square_center(square)
                if 50 < cX < 200:
            ...
            # 创建三组
            group_size = max(len(centers) // 3, 1)  # Ensure group_size is at least 1
            for i in range(0, len(centers_sorted_by_y), group_size):
                y_groups.append(centers_sorted_by_y[i:i + group_size])
        ...
        move = fifth_task(board,bisheng_flag,judge_mid_or_bian)
        print('Move to',move)
